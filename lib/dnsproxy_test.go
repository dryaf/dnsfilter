...
`
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Create a proxy instance for each test
			proxy := newTestProxyWithMocks(t, baseConfig+tc.configExt, map[string]dnsExchanger{
				"resolver_unfiltered":   mockUnfiltered,
				"resolver_anti_malware": mockMalware,
				"resolver_anti_ads":     mockAds,
			})

			msg, err := proxy.ResolveDomain(context.Background(), tc.domain, 1)
			if err != nil && msg == nil {
...
// newTestProxyWithMocks is a helper to create a DNSProxy and inject mock clients.
func newTestProxyWithMocks(t *testing.T, configYAML string, mocks map[string]dnsExchanger) *DNSProxy {
	t.Helper()
	tmpDir := t.TempDir()
	...
	// Create proxy from file first
	proxy, err := NewDNSProxy(configFile, "")
	if err != nil {
		t.Fatalf("Failed to create DNSProxy: %v", err)
	}

	// Now, inject the mocks
	for name, mockClient := range mocks {
		if resolver, ok := proxy.Config.Resolvers[name]; ok {
