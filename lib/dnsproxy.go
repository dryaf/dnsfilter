...
// Proxy is an interface that defines the behavior of the DNS proxy service.
type Proxy interface {
	Run() error
}

// Ensure DNSProxy implements the Proxy interface.
var _ Proxy = (*DNSProxy)(nil)
...
func NewDNSProxy(configFile, listenAddr string) (*DNSProxy, error) {
	if configFile == "" {
		panic("no config.yml")
	}
	...
	return &DNSProxy{Config: config}, nil
}
...
func (p *DNSProxy) Resolve(w dns.ResponseWriter, r *dns.Msg) {
...
func (p *DNSProxy) ResolveDomain(ctx context.Context, domain string, originalMsgID uint16) (*dns.Msg, error) {
	m := new(dns.Msg)
